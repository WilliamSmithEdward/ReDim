...
def test_control_keys(run_widgets):
    ...
    assert facts["radioLetter"] == "Medium", "a letter must jump to the item it starts"
    assert facts["radioPageUp"] == "Small", "Page Up in a short group goes to the first row"
    assert facts["radioPageDown"] == "Large", "Page Down in a short group goes to the last row"
    ...
    assert facts["sliderTyped"] == "35", "digits typed on a slider must set its value"
