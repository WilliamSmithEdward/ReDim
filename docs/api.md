...
| `SetTheme theme` | Restyle every component, and repaint the canvas background if `PrepareCanvas` painted one. Any change restyles what it touches: a border or font alone, and a theme changed in place with its `With` builders and passed to `SetTheme` again. Before the first `Render` it only records the theme, so a build may set it first. |
